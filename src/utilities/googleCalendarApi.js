...
googleCalendarApi.interceptors.response.use(
    ...
    error => {
        ...
        if (!error.response) {
            console.error('[Google Calendar API Error]: Verify your internet connection.');
            return Promise.reject(
                new Error(
                    "Google Calendar API is unreachable. Check your connection",
                ),
      );
        }
        // Handle Authentication errors
        if (error.response?.status === 401) {
            console.warn('[Google Calendar API Error]: Access token may have expired. Please re-authenticate.');
            return Promise.reject(
                new Error(
        ...
        console.error(`[Google Calendar API Error ${error.response?.status}]: ${error.response?.data?.error?.message}`)
        return Promise.reject(
                new Error(
